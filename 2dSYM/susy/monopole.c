...
void monopole() {
  register int i, dir, dir2, ip2;
  register site *s;
  int total_mono_p[2], total_mono_m[2], total, total_abs, *charge[NUMLINK];
...
//        mono[dir2][dir][i] = -mono[dir][dir2][i];
    ...
    if (i == 0) {
      printf("ZZZ (%d, %d) ", s->x, s->t);
      printf("%.6g - %.6g + %.6g - %.6g = %.6g --> %d\n",
          phase[TUP][i], phase[XUP][i], p2, p3,
          total_phase, mono[i]);
    }
  }
  ...
  for (dir = 0; dir < NUMLINK; dir++) {
    ...
    dir2 = NUMLINK - 1 - dir;
    mtag0 = start_gather_field(mono, sizeof(int),
