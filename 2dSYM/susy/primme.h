...
typedef struct primme_params {
  ...
  void (*applyPreconditioner)(void *x,  void *y, int *blockSize,
                              struct primme_params *primme);

  // Matrix times a multivector for mass matrix B for generalized Ax = xBl
  void (*massMatrixMatvec)(void *x,  void *y, int *blockSize,
                           struct primme_params *primme);
  ...
  int nLocal;
  void *commInfo;
  void (*globalSumDouble)(void *sendBuf, void *recvBuf,
                          int *count, struct primme_params *primme );
