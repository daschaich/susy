...
void init_debug_funmatrix(){
  ...
  for(i=1;i<3;i++){
    ...
    for(j=1;j<3;j++) debugMat.e[i][j]=i*j;
  }
...
void fun_mat_scalar_mult( matrix *b, complex s, funmatrix *c) {
  register int i, j, k;
  for(i = 0; i < NCOL; i++)
    for(j = 0; j < NCOLF; j++)
      for( k = 0; k < NCOL; k++){
...
void funa_mat_prod(funmatrix *a, matrix *b, funmatrix *c){
  register int i, j, k;
  for(i = 0; i < NCOL; i++)
    for(j = 0; j < NCOLF; j++)
      for( k = 0; k < NCOL; k++){
...
void funa_mat_prod_dif(funmatrix *a, matrix *b, funmatrix *c){
  register int i, j, k;
  for(i = 0; i < NCOL; i++)
    for(j = 0; j < NCOLF; j++)
      for( k = 0; k < NCOL; k++){
...
void funa_mat_scalar_mult( matrix *b, complex s, funmatrix *c) {
  register int i, j, k;
  for(i = 0; i < NCOL; i++)
    for(j = 0; j < NCOLF; j++)
      for( k = 0; k < NCOL; k++){
