...
void staple_hyp(int dir, int dir2, matrix *lnk1,
  ...
  tag0 = start_gather_field(lnk2, sizeof(matrix), goffset[dir],
                            EVENANDODD, gen_pt[0]);

  // Get blocked_link[dir] from direction dir2
  tag1 = start_gather_field(lnk1, sizeof(matrix), goffset[dir2],
                            EVENANDODD, gen_pt[1]);
  ...
  FORALLSITES(i, s) {
    ...
    mult_na_sum(&tmat1, (matrix *)gen_pt[0][i], stp + i);
  }
    ...
    FORALLUPDIR(dir2) {
      ...
      if (dir != dir2) {
       FORALLSITES(i, s)
        ...
        FORALLSITES(i, s) {
          // Make Omega
          scalar_mult_add_matrix(thin_link[dir] + i,
                                   tempmat + i, ftmp1, &tmat);
          scalar_mult_matrix(&tmat, ftmp2, &Omega);
...
// Overwrite s->link and save original values in thin_link field
void block_hyp() {
  register int i, dir;
  register site *s;
