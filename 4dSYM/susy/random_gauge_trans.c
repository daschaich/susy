...
void random_gauge_trans(Twist_Fermion *TF) {
  ...
  complex tc;
  matrix_f Gmat, tmat, etamat, psimat[NUMLINK], chimat[NPLAQ];
  ...
  // Set up random gaussian matrix_f, then unitarize it
  clear_mat_f(&tmat);
  for (j = 0; j < DIMF; j++) {
#ifdef SITERAND
...
//  mat_copy_f(&tmat, &Gmat);
  ...
  for (j = 0; j < DIMF; j++) {
    mult_nn_f(&(Lambda[j]), &etamat, &tmat);
    tc = trace_f(&tmat);
  ...
  for (j = 0; j < DIMF; j++) {
    mult_nn_f(&etamat, &(Lambda[j]), &tmat);
    tc = trace_f(&tmat);
    CNEGATE(tc, TF[s].Fsite.c[j]);
  }
  ...
  for (a = XUP; a < NUMLINK; a++) {
    mult_nn_f(&Gmat, &(lattice[s].linkf[a]), &tmat);
    mat_copy_f(&tmat, &(lattice[s].linkf[a]));

    clear_mat_f(&(psimat[a]));
    for (j = 0; j < DIMF; j++) {
      mult_nn_f(&Gmat, &(Lambda[j]), &tmat);
      tc = TF[s].Flink[a].c[j];
  ...
  mult_na_f(&(lattice[s].linkf[0]), &Gmat, &tmat);
  mat_copy_f(&tmat, &(lattice[s].linkf[0]));
  clear_mat_f(&(psimat[0]));
  for (j = 0; j < DIMF; j++) {
    mult_na_f(&(Lambda[j]), &Gmat, &tmat);
    tc = TF[s].Flink[0].c[j];
  ...
  mult_na_f(&(lattice[s].linkf[1]), &Gmat, &tmat);
  mat_copy_f(&tmat, &(lattice[s].linkf[1]));
  clear_mat_f(&(psimat[1]));
  for (j = 0; j < DIMF; j++) {
    mult_na_f(&(Lambda[j]), &Gmat, &tmat);
    tc = TF[s].Flink[1].c[j];
  ...
  mult_na_f(&(lattice[s].linkf[2]), &Gmat, &tmat);
  mat_copy_f(&tmat, &(lattice[s].linkf[2]));
  clear_mat_f(&(psimat[2]));
  for (j = 0; j < DIMF; j++) {
    mult_na_f(&(Lambda[j]), &Gmat, &tmat);
    tc = TF[s].Flink[2].c[j];
  ...
  mult_na_f(&(lattice[s].linkf[3]), &Gmat, &tmat);
  mat_copy_f(&tmat, &(lattice[s].linkf[3]));
  clear_mat_f(&(psimat[3]));
  for (j = 0; j < DIMF; j++) {
    mult_na_f(&(Lambda[j]), &Gmat, &tmat);
    tc = TF[s].Flink[3].c[j];
  ...
  mult_na_f(&(lattice[s].linkf[4]), &Gmat, &tmat);
  mat_copy_f(&tmat, &(lattice[s].linkf[4]));
  clear_mat_f(&(psimat[4]));
  for (j = 0; j < DIMF; j++) {
    mult_na_f(&(Lambda[j]), &Gmat, &tmat);
    tc = TF[s].Flink[4].c[j];
