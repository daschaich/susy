...
void initialize_TF() {
  ...
  // Begin with pure Gaussian random number
  FORALLSITES(i, s) {
    for (j = 0; j < DIMF; j++) {
#ifdef SITERAND
      s->F.Fsite.c[j].real = gaussian_rand_no(&(s->site_prn));
      s->F.Fsite.c[j].imag = gaussian_rand_no(&(s->site_prn));
#else
      s->F.Fsite.c[j].real = gaussian_rand_no(&node_prn);
      s->F.Fsite.c[j].imag = gaussian_rand_no(&node_prn);
#endif
      for (mu = 0; mu < NUMLINK; mu++) {
#ifdef SITERAND
        s->F.Flink[mu].c[j].real = gaussian_rand_no(&(s->site_prn));
        s->F.Flink[mu].c[j].imag = gaussian_rand_no(&(s->site_prn));
#else
        s->F.Flink[mu].c[j].real = gaussian_rand_no(&node_prn);
        s->F.Flink[mu].c[j].imag = gaussian_rand_no(&node_prn);
#endif
      }
      for (mu = 0; mu < NUMLINK; mu++) {
        s->F.Fplaq[mu][mu].c[j].real = 0.0;
        s->F.Fplaq[mu][mu].c[j].imag = 0.0;
        for (nu = mu + 1; nu < NUMLINK; nu++) {
#ifdef SITERAND
          s->F.Fplaq[mu][nu].c[j].real = gaussian_rand_no(&(s->site_prn));
          s->F.Fplaq[mu][nu].c[j].imag = gaussian_rand_no(&(s->site_prn));
#else
          s->F.Fplaq[mu][nu].c[j].real = gaussian_rand_no(&node_prn);
          s->F.Fplaq[mu][nu].c[j].imag = gaussian_rand_no(&node_prn);
#endif
          CNEGATE(s->F.Fplaq[mu][nu].c[j], s->F.Fplaq[nu][mu].c[j]);
        }
      }
    }
  }
...
void initialize_p_F() {
  ...
  // Begin with pure Gaussian random number
  node0_printf("initializing p_F\n");
  fflush(stdout);

  FORALLSITES(i, s) {
    for (j = 0; j < DIMF; j++) {
#ifdef SITERAND
      s->p_F.Fsite.c[j].real = gaussian_rand_no(&(s->site_prn));
      s->p_F.Fsite.c[j].imag = gaussian_rand_no(&(s->site_prn));
#else
      s->p_F.Fsite.c[j].real = gaussian_rand_no(&node_prn);
      s->p_F.Fsite.c[j].imag = gaussian_rand_no(&node_prn);
#endif
      for (mu = 0; mu < NUMLINK; mu++) {
#ifdef SITERAND
        s->p_F.Flink[mu].c[j].real = gaussian_rand_no(&(s->site_prn));
        s->p_F.Flink[mu].c[j].imag = gaussian_rand_no(&(s->site_prn));
#else
        s->p_F.Flink[mu].c[j].real = gaussian_rand_no(&node_prn);
        s->p_F.Flink[mu].c[j].imag = gaussian_rand_no(&node_prn);
#endif
      }
     for (mu = 0; mu < NUMLINK; mu++) {
        s->p_F.Fplaq[mu][mu].c[j].real = 0.0;
        s->p_F.Fplaq[mu][mu].c[j].imag = 0.0;
        for (nu = mu + 1; nu < NUMLINK; nu++) {
#ifdef SITERAND
          s->p_F.Fplaq[mu][nu].c[j].real = gaussian_rand_no(&(s->site_prn));
          s->p_F.Fplaq[mu][nu].c[j].imag = gaussian_rand_no(&(s->site_prn));
#else
          s->p_F.Fplaq[mu][nu].c[j].real = gaussian_rand_no(&node_prn);
          s->p_F.Fplaq[mu][nu].c[j].imag = gaussian_rand_no(&node_prn);
#endif
          CNEGATE(s->p_F.Fplaq[mu][nu].c[j], s->p_F.Fplaq[nu][mu].c[j]);
        }
      }
    }
  }
