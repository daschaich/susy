...
double local_plaquette(double *ss_plaq, double *st_plaq) {
...
#endif
#ifdef PLAQ_DIST
          printf("PLAQ_DIST %d %d %d %d %d %.4g\n",
                 s->x, s->y, s->t, dir, dir2, cur_plaq);
#endif
          st_sum += cur_plaq;
#ifdef LOCAL_PLAQ
  ...
  *st_plaq = st_sum / ((double)(3.0 * volume));
  ...
  // Normalization
  for (xx = 0; xx < MY_N; xx++) {
    plaq_perp[xx] *= ((double)(MY_N)) / (2.0 * volume):
    plaq_prll[xx] *= ((double)(MY_N)) / (2.0 * volume);
  }

  // Print out
  if (this_node == 0) {
    if (print_dir == 0) {
      printf("LOCAL_PLAQ [0=XUP,..., 2=TUP] dir=%d\n", MY_DIR);
      print_dir = 1;
    }
