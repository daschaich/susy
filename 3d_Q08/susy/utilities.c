  ...
  for (j = 0; j < NTERMS ; j++) {
    ...
    if (j < NTERMS - 1) {
      ...
      tag0[gather] = start_gather_site(F_OFFSET(link[c]), sizeof(matrix),
                              DbVP_d1[next], EVENANDODD, local_pt[gather][0]);
      tag1[gather] = start_gather_site(F_OFFSET(link[c]), sizeof(matrix),
                              goffset[c] + 1, EVENANDODD, local_pt[gather][1]);
      tag2[gather] = start_gather_field(src, sizeof(matrix),
                              goffset[c] + 1, EVENANDODD, local_pt[gather][2]);
    }
    a = DbVtoP_lookup[j][0];
    b = DbVtoP_lookup[j][1];
    c = DbVtoP_lookup[j][2];
    ...
    FORALLSITES(i, s) {
      ...
      scalar_mult_an_dif((matrix *)(local_pt[flip][1][i]),
...
void DbplusPtoV(matrix *src[NPLAQ], matrix *dest) {
  ...
  register site *s;
  char **local_pt[2][2];
  int a, b, c, j, i_ab, gather, next, flip = 0;
  Real tr;
  msg_tag *tag0[2], *tag1[2];
  ...
  for (j = 0; j < NTERMS ; j++) {
    ...
    if (j < NTERMS - 1) {
      ...
      tag1[gather] = start_gather_site(F_OFFSET(link[c]), sizeof(matrix),
                               DbVP_d1[next], EVENANDODD, local_pt[gather][1]);
    }
    a = DbVtoP_lookup[j][0];
    b = DbVtoP_lookup[j][1];
    c = DbVtoP_lookup[j][2];
...
void fermion_op(Twist_Fermion *src, Twist_Fermion *dest, int sign) {
  ...
  if (sign == 1) {
    FORALLSITES(i, s) {
      ...
      mat_copy(&(src[i].Fvolume), &(volume_src[i]));
      FORALLDIR(mu)
    ...
    FORALLSITES(i, s) {
      ...
      adjoint(&(src[i].Fvolume), &(volume_src[i]));
      FORALLDIR(mu)
  ...
  // Copy local volume, plaquette, link and site fermions into dest TwistFermion
  if (sign == 1) {
    FORALLSITES(i, s) {
      mat_copy(&(site_dest[i]), &(dest[i].Fsite));
      mat_copy(&(volume_dest[i]), &(dest[i].Fvolume));
      FORALLDIR(mu)
    ...
    FORALLSITES(i, s) {
      ...
      neg_adjoint(&(volume_dest[i]), &(dest[i].Fvolume));
      FORALLDIR(mu)
