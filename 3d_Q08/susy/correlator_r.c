...
// -----------------------------------------------------------------
// Map (x, y, t) to r on Nx x Ny x Nt A3* lattice
// Check all possible periodic shifts to find true r
Real A3map(int x_in, int y_in, int t_in) {
  int x, y, t, xSq, ySq, xy, xpy;
  ...
  for (x = x_in - nx; x <= x_in + nx; x += nx) {
    ...
    for (y = y_in - ny; y <= y_in + ny; y += ny) {
      ...
      for (t = t_in - nt; t <= t_in + nt; t += nt) {
        tr = sqrt((xSq + ySq + t * t) * 0.8 - xy * 0.4);
        if (tr < r)
          r = tr;
      }
    }
...
int count_points(Real MAX_r, Real *save, int *count, int MAX_pts) {
  int total = 0, this, j, x_dist, y_dist, t_dist;
  int y_start, t_start;
  Real tr;
  ...
  for (x_dist = 0; x_dist <= MAX_X; x_dist++) {
    ...
    for (y_dist = y_start; y_dist <= MAX_X; y_dist++) {
      // Don't need negative t_dist when x and y are all non-positive
...
void correlator_r() {
  register int i;
  register site *s;
  int a, b, j, x_dist, y_dist, t_dist;
  int y_start, t_start, this_r, total_r = 0;
  ...
  for (x_dist = 0; x_dist <= MAX_X; x_dist++) {
    ...
    for (y_dist = y_start; y_dist <= MAX_X; y_dist++) {
      ...
      for (t_dist = t_start; t_dist <= MAX_X; t_dist++) {
        ...
        node0_printf("ROT_K %d %d %d", x_dist, y_dist, t_dist);
#endif
        for (a = 0; a < N_K; a++) {
          for (b = 0; b < N_K; b++) {
            tr = 0.0;
...
#ifdef CHECK_ROT
        // Potentially useful to check rotational invariance
        node0_printf("\nROT_S %d %d %d", x_dist, y_dist, t_dist);
#endif
        for (a = 0; a < N_K; a++) {
          for (b = 0; b < N_K; b++) {
            tr = 0.0;
