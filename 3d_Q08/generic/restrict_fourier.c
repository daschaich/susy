// -----------------------------------------------------------------
// Set up and perform Fourier transforms on fields in the lattice
// Permits some coordinates to be fixed
...
int logdim[3];    /* log_2 of dimension */
int bitrev_dir;   /* index for bit reverse gather */
int *butterfly_dir[3];  /* indices for butterflies.  First index is
...
int pcyclic_dir[3];     /* index for cyclic mod p gather */
int dmin[3], dmax[3];   /* Restrictions on range of FT */
int pfactor[3];         /* Residual prime factor p */
...
void setup_restrict_fourier( int *key, int *slice) {
  ...
  int arg[2];
  void bitrev_map(int x, int y, int t, int *key, int fb,
                  int *xp, int *yp, int *tp);
  void butterfly_map(int x, int y, int t, int *arg, int fb,
                     int *xp, int *yp, int *tp);
  void pbaserev_map(int x, int y, int t, int *key, int fb,
                    int *xp, int *yp, int *tp);
  void pcyclic_map(int x, int y, int t, int *arg, int fb,
                   int *xp, int *yp, int *tp);

  /* Initialize limits on coordinate range of FT */
  dmin[XUP] = dmin[YUP] = min[TUP] = 0;
  dmax[XUP] = nx; dmax[YUP] = ny; dmax[TUP] = nt;
     ...
     int isign)    /* 1 for x -> k, -1 for k -> x */
{
  ...
  for (t=dmin[TUP]; t < dmax[TUP]; t++)
    {
      if ( node_number(x,y,t) != mynode() )continue;
      i = node_index(x,y,t);
      s = &lattice[i];
...
/**  for (x=0;x<nx;x++)for (y=0;y<ny;y++)for (z=0;z<nz;z++)for (t=0;t<nt;t++)
      ...
      for (t=dmin[TUP]; t < dmax[TUP]; t++)
  {
    ...
    i = node_index(x,y,t);
    s = &lattice[i];
    /* Find coordinate - treat s->x,y,t as array */
    if (dir==XUP)n = s->x;
    else if (dir==YUP)n = s->y;
    else if (dir==TUP)n = s->t;
