...
// sort_six_gathers()   Sort eight contiguous gathers
...
msg_tag* start_general_strided_gather(
  ...
  int stride,           /* bytes between fields in source buffer */
  int size,   /* size in bytes of the field (eg sizeof(vector))*/
  int *displacement,  /* displacement to gather from. three components */
  ...
  char **dest)   /* one of the vectors of pointers */
{
  ...
  if ((displacement[XUP] + displacement[YUP] +
