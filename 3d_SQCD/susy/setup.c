...
int initial_set() {
  ...
  if (mynode() == 0) {
    FORALLUPDIR(dir) {
      if (length[dir] < 1) {
        printf("{nx, ny, nt} must all be positive\n");
        exit(1);
      }
    }
  }
...
void make_fields() {
  ...
  FIELD_ALLOC(volume_src, matrix);
  FIELD_ALLOC(volume_dest, matrix);
  ...
  size += (Real)(NUMLINK) * sizeof(funmatrix);
  FIELD_ALLOC_VEC(Fmu5, funmatrix, NUMLINK);
