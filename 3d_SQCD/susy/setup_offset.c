...
void setup_qclosed_offset() {
  ...
  //Set up gather for the various n-1-2-3 terms
  FORALLUPDIR(i) {
    d1[i] = - mu_vec[0][i] - mu_vec[1][i] - mu_vec[2][i];
  }
  d1_list = check_list(d1, &minus);
  if (minus > 0)
    allminus_offset = gq_offset[d1_list];
  if (minus < 0)
    allminus_offset = gq_offset[d1_list]+1;
#ifdef DEBUG_CHECK
  node0_printf("-1-2-3 general gather ");
  node0_printf("(%d %d %d), %d, %d\n",
               d1[0], d1[1], d1[2],
               d1_list, allminus_offset);
#endif
}
// -----------------------------------------------------------------
