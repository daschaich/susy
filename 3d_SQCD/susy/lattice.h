...
typedef struct {
  matrix Fsite; //scalar
  matrix Flink[NUMLINK]; //vector
  matrix Fplaq[NPLAQ]; //anti-sym tensor
  matrix Fvolume; //volume form
  //TODO: remove Fvolume and the force/action term associated with it.
  // More to be added as needed, not at the beginning
} Twist_Fermion;
// -----------------------------------------------------------------
...
typedef struct {
  ...
  funmatrix funlink;          // Fundamental link living on G slice
  funmatrix varlink;       // Fundamental link living on F slice
  // Living on X slice refers to the transformation properties of the links

#ifdef HMC_ALGORITHM
  matrix old_link[NUMLINK];   // For accept/reject
  funmatrix old_funlink;      // For accept/reject
  funmatrix old_funvarlink;   // For accept/reject
#endif
  ...
  funmatrix funmom, f_phi;                  // Force matrices for phi
  funmatrix varmom, f_var;                  // Force matrices for varphi
...
EXTERN int length[3];       // Duplicate lattice dimensions for loops
...
//Gather for -1-2-3 direction that appears frequently
EXTERN int allminus_offset;

EXTERN int DbVP_d1[NTERMS];
EXTERN int DbVtoP_lookup[NTERMS][NUMLINK];
//EXTERN int DbplusPtoP_lookup[NTERMS][NUMLINK];
//EXTERN int DbminusPtoP_lookup[NTERMS][NUMLINK];

EXTERN int FQ_d1[NTERMS];
...
EXTERN matrix *site_src, *link_src[NUMLINK], *plaq_src[NPLAQ], *volume_src;
EXTERN matrix *site_dest, *link_dest[NUMLINK], *plaq_dest[NPLAQ], *volume_dest;
