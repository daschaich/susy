...
// -----------------------------------------------------------------
// Calculating cV_a varphi(n+a) - varphi(n)U(n-1-2-3) at each site
// as it will be used for both action and force
// Using tempfunmat for temporary storage
void compute_Fmu5() {
#ifdef VARPHI
  register int i;
  register site *s;
  ...
  FORALLDIR(mu) {
    mtag0 = start_gather_site(F_OFFSET(link[mu]), sizeof(matrix),
                              allminus_offset, EVENANDODD, gen_pt[0]);
    mtag1 = start_gather_site(F_OFFSET(varlink), sizeof(funmatrix),
    ...
    FORALLSITES(i ,s) {
      mat0 = (matrix *)(gen_pt[0][i]);
      funmat0 = (funmatrix *)(gen_pt[1][i]);
      
      fun_ta_mult_nn(&(s->varlink), mat0, &(tempfunmat[i]));
      fun_o_sub(funmat0, &(tempfunmat[i]), &(Fmu5[mu][i]));
    }
...
// -----------------------------------------------------------------
// Contribution due to varphi (no phi) to the action
double var_action(int do_det) {
#ifdef VARPHI
  register int i, mu;
  register site *s;
  double v_action = 0.0;
  ...
  FORALLSITES(i, s) {
    clear_mat(&(tmat));
    FORALLDIR(mu) fun_tt_mult_an_sum(&(Fmu5[mu][i]), &(Fmu5[mu][i]), &(tmat));
...
#ifdef VARPHI
    sum += (double)fun_nn_realtrace(&(s->varmom),&(s->varmom));
#endif
  }
  g_doublesum(&sum);
  return sum;
...
double action(Twist_Fermion **src, Twist_Fermion ***sol) {
  double g_act, v_act, bmass_act = 0.0, p_act, f_act, det_act = 0.0;
  double cWline_act = 0.0, total;

  g_act = gauge_action(NODET);
  v_act = var_action(NODET);
  if (bmass > IMAG_TOL)
  ...
  node0_printf("action: ");
  node0_printf("gauge %.8g varphi %.8g bmass %.8g ", g_act, v_act, bmass_act);
  node0_printf("det %.8g ", det_act);

  total = g_act + v_act + bmass_act + det_act + cWline_act;
#ifndef PUREGAUGE
  int n;
  for (n = 0; n < Nroot; n++) {
