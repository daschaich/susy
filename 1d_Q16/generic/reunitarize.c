...
void reunit_report_problem_matrix(matrix *mat, int i, int j) {
  ...
  printf("Unitarity problem on node %d, site %d, field %d tolerance=%e\n",
...
void reunitarize() {
  ...
  FORALLSITES(i, s) {
    ...
    if (errors)reunit_report_problem_matrix(mat, i, -1);
    ...
    for(j=0;j<NSCALAR;j++) {
      ...
      errors = reunit(mat);
      errcount += errors;
      if (errors)reunit_report_problem_matrix(mat, i, j);
      if (errcount > MAXERRCOUNT) {
        printf("Unitarity error count exceeded: %d\n", errcount);
        fflush(stdout);
        terminate(1);
